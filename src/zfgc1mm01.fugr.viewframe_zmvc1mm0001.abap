*---------------------------------------------------------------------*
*    program for:   VIEWFRAME_ZMVC1MM0001
*---------------------------------------------------------------------*
FUNCTION VIEWFRAME_ZMVC1MM0001         .
...
*----------------------------------------------------------------------*
CALL FUNCTION 'VIEWPROC_ZMVC1MM0001'
         EXPORTING
              FCODE          = READ
...
EXTRACT = ZMVC1MM0001_EXTRACT
TOTAL = ZMVC1MM0001_TOTAL
              X_HEADER       = X_HEADER
              X_NAMTAB       = X_NAMTAB
...
CALL FUNCTION 'VIEWPROC_ZMVC1MM0001'
             ...
             IMPORTING
                  UCOMM           = FUNCTION
UPDATE_REQUIRED = STATUS_ZMVC1MM0001-UPD_FLAG
             TABLES
                  EXCL_CUA_FUNCT  = EXCL_CUA_FUNCT
EXTRACT = ZMVC1MM0001_EXTRACT
TOTAL = ZMVC1MM0001_TOTAL
...
*----------------------------------------------------------------------*
      ...
      IF ( FUNCTION EQ SWITCH_TO_SHOW_MODE OR
           ...
           FUNCTION EQ END ) AND
STATUS_ZMVC1MM0001-UPD_FLAG NE SPACE.
        PERFORM BEENDEN.
        CASE SY-SUBRC.
          WHEN 0.
CALL FUNCTION 'VIEWPROC_ZMVC1MM0001'
...
TOTAL = ZMVC1MM0001_TOTAL
            ...
            CASE SY-SUBRC.
              WHEN 0.
IF STATUS_ZMVC1MM0001-UPD_FLAG EQ SPACE. EXIT. ENDIF.
...
IF STATUS_ZMVC1MM0001-UPD_FLAG NE SPACE.
